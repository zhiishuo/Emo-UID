...
from trains.singleTask.model import uid
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    if args.is_distill:
        ...
        model = []
        model_uid = getattr(uid, 'uid')(args)
        ...
        model_uid, model_distill_homo, model_distill_hetero = model_uid.cuda(), model_distill_homo.cuda(), model_distill_hetero.cuda()

        model = [model_uid, model_distill_homo, model_distill_hetero]
    else:
        print("testing phase for uid")
        model = getattr(uid, 'uid')(args)
        model = model.cuda()
    ...
    if args.mode == 'test':
        ...
        model[0].load_state_dict(torch.load('pt/uid.pth'))
